...
export default function ProcessManager({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
      {/* 1. Add Process Form */}
      <div className="theme-bg-card p-5 rounded-2xl border theme-border shadow-xl flex flex-col justify-between h-full">
        <div>
          <h2 className="text-sm font-bold theme-text mb-4 flex items-center justify-between font-sans">
            <span className="uppercase tracking-widest text-xs flex items-center gap-2">
              <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-pulse"></span>
              Add Simulation Process
            </span>
            <span className="text-[9px] theme-bg-tag theme-text-tag border theme-border px-2.5 py-1 rounded-full font-mono font-bold uppercase tracking-wider">
              Inputs
            </span>
          </h2>
          ...
          <form onSubmit={handleAddProcess} className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold theme-text-secondary uppercase tracking-wider mb-1.5 font-mono">
              ...
              <input
                ...
                className="w-full text-xs theme-text theme-bg-input border theme-border-input focus:border-indigo-500 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-indigo-500/20 transition-all font-sans"
                placeholder="e.g. P1"
                required
              />
            </div>
            ...
            {errorMsg && (
              <div className="text-red-400 text-xs flex items-center gap-1.5 bg-red-500/5 p-2.5 rounded-xl border border-red-500/20 animate-pulse font-sans">
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-700 border border-indigo-500/20 text-white font-bold text-xs py-3 rounded-xl cursor-pointer flex items-center justify-center gap-1.5 shadow-[0_0_15px_rgba(79,70,229,0.15)] hover:shadow-[0_0_20px_rgba(79,70,229,0.25)] transition-all uppercase tracking-wider"
            >
              <Plus size={14} />
        ...
        <div className="mt-5 border-t theme-border pt-4 flex gap-2 font-sans">
          ...
          <button
            ...
            className="theme-bg-tag border theme-border hover:text-red-400 hover:border-red-400/30 px-4 py-2 rounded-xl text-xs font-bold theme-text-secondary cursor-pointer transition-colors"
          >
            Clear All
          </button>
        </div>
      ...
      <div className="theme-bg-card p-5 rounded-2xl border theme-border shadow-xl flex flex-col justify-between">
        <div>
          <h2 className="text-sm font-bold theme-text mb-4 flex items-center justify-between font-sans uppercase tracking-widest">
          ...
          <div className="space-y-5 text-xs">
            <div>
              <label className="block theme-text-secondary font-bold mb-1.5 uppercase font-mono tracking-wider text-[9px]">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="w-full accent-indigo-500 h-1 theme-bg-input rounded-lg cursor-pointer border theme-border"
                />
                <span className="text-sm font-mono font-bold theme-bg-input text-indigo-400 px-3 py-1 rounded-xl border theme-border min-w-10 text-center shadow-inner">
                  {rrQuantum}
                </span>
              </div>
              <p className="text-[10px] theme-text-secondary mt-2 font-sans leading-relaxed">
                Specifies maximum contiguous CPU-burst duration allowed before a process is context-switched in Round Robin.
              </p>
            </div>

            <div className="border-t theme-border pt-4">
              <label className="block theme-text-secondary font-bold mb-2 uppercase font-mono tracking-wider text-[9px]">
                Priority Assignment Mapping
              </label>
              <div className="grid grid-cols-1 gap-2 theme-bg-input p-1.5 rounded-xl border theme-border font-sans">
                <button
                  type="button"
                  onClick={() => onUpdateLowerIsHigher(true)}
                  className={`text-[11px] py-2 px-3 rounded-lg font-semibold transition-all ${
                    lowerIsHigher
                      ? "bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 font-bold shadow-sm"
        ...
        <div className="theme-bg-inset p-3.5 rounded-xl border theme-border text-[10px] text-indigo-400 font-sans mt-4 leading-relaxed">
          <div className="font-bold theme-text mb-0.5 uppercase tracking-wider text-[9px]">Multilevel Queue Rule:</div>
      ...
      {/* 3. Processes Table */}
      <div className="theme-bg-card p-5 rounded-2xl border theme-border shadow-xl flex flex-col h-full lg:max-h-[385px] overflow-hidden">
        <h2 className="text-sm font-bold theme-text mb-3 flex items-center justify-between shrink-0 font-sans uppercase tracking-widest">
          <span>Active Ready Queue ({processes.length})</span>
          <span className="text-[9px] bg-emerald-500/10 text-emerald-500 border border-emerald-500/25 px-2.5 py-1 rounded-full font-bold">
            Ready Queue
          </span>
        </h2>

        {processes.length === 0 ? (
          <div className="flex-1 border-2 border-dashed theme-border rounded-2xl flex flex-col items-center justify-center p-6 text-center theme-text-secondary">
            <Shuffle size={20} className="theme-text-muted mb-2 animate-pulse" />
            <p className="text-xs font-semibold theme-text">Ready Queue Empty</p>
            <p className="text-[10px] theme-text-muted max-w-xs mt-1 leading-relaxed">Click "Random Workload" or add processes above to begin.</p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto border theme-border rounded-xl scrollbar-none">
            <table className="w-full text-left text-xs">
              <thead className="theme-bg-inset theme-text-secondary uppercase text-[9px] tracking-wider sticky top-0 border-b theme-border">
                <tr>
              ...
              <tbody className="divide-y theme-border-subtle font-mono">
                {processes.map((p) => (
                  <tr key={p.pid} className="hover:theme-bg-card-subtle transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-right">
                      <button
                        ...
                        className="theme-text-muted hover:text-red-400 p-1 rounded-md cursor-pointer transition-colors"
