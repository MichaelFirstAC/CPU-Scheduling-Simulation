...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 pb-16 flex flex-col font-sans select-none antialiased">
      {/* 1. COMP6697001 Formal Academic Head Banner styled like a Bento item */}
      <header className="bg-slate-900/30 border-b border-slate-800 select-none">
        <div className="max-w-7xl mx-auto px-6 py-6 md:py-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-300 px-2.5 py-1 rounded-md border border-amber-500/20 font-mono flex items-center gap-1">
                <Sparkles size={10} />
                <span>Final course project</span>
              </span>
              <span className="text-[10px] font-bold uppercase tracking-wider bg-emerald-950/60 text-slate-300 px-2.5 py-1 rounded-md border border-green-900/50 font-mono">
          ...
          <div className="shrink-0 text-left md:text-right text-xs text-slate-400 border-l border-slate-800 md:border-l-0 md:border-r border-slate-800 pl-4 md:pl-0 md:pr-4 space-y-0.5 font-mono">
            <div><strong className="text-slate-300 font-semibold">Lecturer:</strong> Dr. Satrio Pradono Suryodiningrat</div>
            <div><strong className="text-indigo-400 font-semibold">University:</strong> Binus International University</div>
            <div><strong className="text-yellow-400 font-semibold">Student 1:</strong> Michael Arianno C / 2802499711</div>
            <div><strong className="text-yellow-400 font-semibold">Student 2:</strong> Timothy Jonathan I / 2802521825</div>
          </div>
        </div>
      </header>

      {/* 2. Persistent Navigation Tabs nested as Bento Panel */}
      <div className="bg-[#020617]/80 backdrop-blur-md border-b border-slate-900 sticky top-0 z-20 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          <nav className="flex space-x-1.5 py-2 md:py-2.5">
            <button
              ...
              className={`text-xs md:text-sm py-2 px-3.5 md:px-4 rounded-lg font-semibold transition-all cursor-pointer flex items-center gap-2 ${currTab === "simulate"
                ? "bg-indigo-600/10 text-indigo-300 border border-indigo-500/30 shadow-[0_0_15px_rgba(79,70,229,0.15)] font-bold"
          ...
          <div className="hidden lg:flex items-center gap-2">
            <span className="text-[10px] text-slate-500 uppercase font-mono">Sim Engine:</span>
            {isSimulating ? (
              <span className="inline-flex items-center gap-1.5 text-[10px] text-slate-450 bg-slate-900/40 border border-slate-800 py-1 px-3 rounded-full font-mono">
                <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-ping"></span>
                <span>Resolving Thread...</span>
              </span>
            ) : calculationSource === "python" ? (
              <span className="inline-flex items-center gap-1.5 text-[10px] text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 py-1 px-3 rounded-full font-mono font-bold">
                <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></span>
                <span>Active Python Runtime Engine</span>
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/30 py-1 px-3 rounded-full font-mono font-bold" title={simError}>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-6 flex-1 w-full space-y-6">
        {simError && currTab === "simulate" && (
          <div className="bg-amber-500/5 border border-amber-500/20 text-amber-300 p-4 rounded-2xl text-xs space-y-1">
            ...
            {processes.length > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
                ...
                <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl">
                    ...
                    {ALGORITHMS.map((algo) => (
                      <button
                        ...
                        className={`w-full text-left text-xs py-3 px-3.5 rounded-xl font-medium transition-all cursor-pointer flex flex-col gap-0.5 border ${activeAlgo === algo.key
                          ? "bg-indigo-600/10 border-indigo-500/30 text-indigo-300 shadow-[0_0_12px_rgba(99,102,241,0.15)] font-bold"
                          : "bg-slate-950/40 border-transparent text-slate-400 hover:bg-slate-900/50 hover:text-slate-200"
                          }`}
                      >
                        <div className="flex items-center justify-between w-full">
                          ...
                          <span className={`text-[8px] font-mono px-1.5 py-0.5 rounded font-bold uppercase tracking-tighter ${algo.type === "core"
                            ? activeAlgo === algo.key ? "bg-indigo-900/50 text-indigo-200" : "bg-slate-800 text-slate-400"
                  ...
                  <div className="border-t border-slate-800 pt-4">
                    <div className="p-4 bg-slate-950/40 border border-slate-850/60 rounded-xl space-y-2">
                      ...
                      <p className="text-[11px] text-slate-400 leading-relaxed font-normal font-sans">
                ...
                <div className="lg:col-span-3 space-y-6">
                  ...
                  <LiveSimulation simulation={activeResult} inputs={processes} />

                  {/* Single Algorithm Wait/Turnaround Metrics Cards */}
                  {activeResult && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-slate-900/30 border border-slate-800 p-4 rounded-xl shadow-xs font-mono flex flex-col justify-between">
                      ...
                      <div className="bg-emerald-950/5 border border-emerald-500/10 p-4 rounded-xl shadow-xs font-mono flex flex-col justify-between">
                        <span className="text-[9px] font-mono uppercase text-emerald-450 flex items-center gap-1">
                      ...
                      <div className="bg-blue-950/5 border border-blue-500/10 p-4 rounded-xl shadow-xs font-mono flex flex-col justify-between">
                        <span className="text-[9px] font-mono uppercase text-blue-405 flex items-center gap-1">
                      ...
                      <div className="bg-indigo-950/5 border border-indigo-500/10 p-4 rounded-xl shadow-xs font-mono flex flex-col justify-between">
                        <span className="text-[9px] font-mono uppercase text-indigo-405 flex items-center gap-1">
                  ...
                  {activeResult && (
                    <div className="bg-slate-900/30 border border-slate-800 rounded-2xl overflow-hidden shadow-2xs">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs whitespace-nowrap">
                          <thead className="bg-slate-950/60 text-slate-400 font-semibold uppercase text-[9px] tracking-wider border-b border-slate-800">
                            <tr>
                          ...
                          <tbody className="divide-y divide-slate-850/60">
                            {activeResult.processes.map((p) => (
                              <tr key={p.pid} className="hover:bg-slate-900/30 transition-colors font-mono">
                                <td className="px-5 py-3 text-slate-100 font-bold">{p.pid}</td>
                                <td className="px-5 py-3 text-center text-slate-400">{p.arrival_time} ms</td>
                                <td className="px-5 py-3 text-center text-slate-400">{p.burst_time} ms</td>
                                <td className="px-5 py-3 text-center text-indigo-400 font-bold">#{p.priority}</td>
                                <td className="px-5 py-3 text-center font-bold text-emerald-400">{p.waiting_time} ms</td>
                                <td className="px-5 py-3 text-center font-bold text-blue-400">{p.turnaround_time} ms</td>
                                <td className="px-5 py-3 text-center text-slate-455">{p.completion_time} ms</td>
                                <td className="px-5 py-3 text-center text-slate-455">{p.response_time} ms</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
              ...
              <div className="bg-[#020617]/50 p-12 rounded-2xl border border-slate-800 text-center text-slate-405 font-medium py-16 flex flex-col items-center justify-center space-y-3 shadow-md">
                <Cpu size={32} className="text-slate-600 stroke-1" />
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-slate-300">Ready List is Empty</p>
                  <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">Please add a process value or generate a random workload above to launch the simulation.</p>
                </div>
              </div>
            )}
          </div>
        ...
        {currTab === "analytics" && (
          <section aria-label="Performance Comparisons">
            <PerformanceDashboard allResults={simulatorResults} activeAlgoKey={activeAlgo} />
          </section>
        )}
